...
- Lines `{rang, prise_id, votes, moi, nom, pseudonyme, avatar}`. `nom` is the first name for a person who opted in and for the caller's own line (they know who they are), « Voix N » otherwise; `pseudonyme` says which; `avatar` is the picture's path only where the name is a real one. The phone draws a crown in gold, silver and bronze on the first three, a play or stop control on the caller's own line, and « (toi) » after their name.
