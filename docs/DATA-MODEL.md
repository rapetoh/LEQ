...
- Lines `{rang, prise_id, votes, moi, nom, pseudonyme, avatar, duree_s, chemin_audio}`. `nom` is the first name for a person who opted in and for the caller's own line (they know who they are), « Passage N » otherwise (2026-09-18: it was « Voix N », and a vote is « une voix », so the ranking read « Voix 2 · 2 voix »; « un passage » is chapter 11's word and the one the app already uses for the person's own take); `pseudonyme` says which; `avatar` is the picture's path only where the name is a real one. `chemin_audio` (2026-09-18) is the path in `audio-public` only where the caller may hear the passage: their own always, the others' once they have spoken on that subject (`a_parle_sur()`), never once the audio is deleted; `duree_s` is the take's length. The phone draws a crown in gold, silver and bronze on the first three, a play or stop control on every line it may hear, and « (toi) » after the caller's name.
