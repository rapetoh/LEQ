...
- Lines `{rang, prise_id, votes, moi, nom, pseudonyme, avatar, duree_s, chemin_audio}`. `nom` is the first name for a person who opted in and for the caller's own line (they know who they are), « Voix N » otherwise; `pseudonyme` says which; `avatar` is the picture's path only where the name is a real one. `chemin_audio` (2026-09-18) is the path in `audio-public` only where the caller may hear the passage: their own always, the others' once they have spoken on that subject (`a_parle_sur()`), never once the audio is deleted; `duree_s` is the take's length. The phone draws a crown in gold, silver and bronze on the first three, a play or stop control on every line it may hear, and « (toi) » after the caller's name.
