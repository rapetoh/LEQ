...
- `avatar_chemin` (text, nullable, 2026-09-17): the person's picture, a path in the public bucket `avatars` shaped `<uid>/<horodatage>.jpg`. A new object per change and the previous one deleted by the phone, so a cached picture never outlives a change. Shown on Moi and Mon compte, and next to the name wherever the name is shown (the ranking, the podium): `classement_arene()` gates `avatar` on exactly the condition that gates `nom`, so the opt-in `publier_sous_prenom` governs the face as it governs the first name, and a pseudonym never carries a face. The switch in Réglages reads « Publier sous mon prénom et ma photo ».
...
| purge_anonymes_heures        | nombre | 720      | Hours without a take or a sign-in before an anonymous account is deleted (thirty days, from the last take, 2026-09-17) |
...
- Job `notifier_duel` `{duel_id, evenement, acteur_id?}` (worker, 2026-09-18): queued by `rejoindre_duel()`, `publier_prise()` and `cloturer_duel()`. The inviter hears `rejoint`, the side that has not spoken hears `repondu`, both hear `verdict` and `expire`; the actor of a gesture is never told about it. A push per active token of a person who keeps `notif_social` on (chapter 12, social events); its data carries `duel_id`, which opens the duel. An invitee who answered by the link (anonymous, `invite_email` set) gets an e-mail at the end of the duel with the outcome in words and the link, through the worker's SMTP (`SMTP_*` secrets, the Gmail app password of the sign-in codes); without SMTP the job logs it and carries on.
...
- `consommation jsonb` (nullable): what the providers consumed for the session, in their own units, written by the server at the close and completed by the debrief job. `{version: 1, transcription: {audio_entree_s, jetons_audio?, jetons_texte?}, retor: {jetons_entree, jetons_caches, jetons_sortie, appels}, voix: {caracteres, audio_s, appels}, debrief?: {same four as retor}}`. No price is stored: prices change, and the cost of a session is read at query time against the day's rates. Null on a session the server never closed itself, which is the honest value: nothing is invented.
- **This table is the quota ledger** (ADR-009): one row per session carrying its own outcome, and the month is counted by replaying them. `quota_debats()` counts the sessions of the current month, in the person's own zone, in which the person spoke (a `tours_debat` row with `locuteur = 'utilisateur'`) and whose `issue` is not `interrompue_par_nous`, open or closed alike (migration 20260918000000). A session in which nobody said a word costs nothing whatever closed it: on 2026-09-17 a debate whose audio never started stayed open with no turn, the chooser read one session left, and each tap closed it as abandoned, counted it, refused for `quota_epuise` and rolled back, forever. A session we cut ourselves keeps its row and costs nothing, because charging a quota for our own outage is the shortest road to refund requests.
...
- `cloturer_debat(debat, issue, session, consommation)` is service role only: the server decides whether a cut was ours, and hands over what the session consumed. A connection that no longer holds the debate closes nothing; a resumed session's last close carries the whole count.
...
- `id`, `utilisateur_id`, `tentative_id unique`, `contexte` (arena or duel, checked against `sujet_id` and `duel_id`), `chemin_audio`, `statut` (publiee on send, signalee when the screening held it for Rebecca, retiree by her decision or the person's own), `retiree_par` (`personne` or `admin`, null while not withdrawn, 2026-09-18), `motif_retrait`, `votes_recus`, `date_suppression`, `audio_supprime_le`, timestamps. One take per person per duel; one take per person per week among the takes that stand (`prises_publiques_une_par_semaine_idx` ignores `retiree`, so a withdrawn passage leaves room for another).
- `retirer_ma_prise(prise)` (authenticated, 2026-09-18): the person withdraws their own Arena passage while the subject is open (`sujet_ferme` otherwise, `prise_introuvable` on anyone else's). Status `retiree`, `retiree_par = 'personne'`, audio marked for deletion. Having withdrawn, the person no longer counts as having spoken (`a_parle_sur()` ignores `retiree`) and may publish another passage. `moderer_prise()` writes `retiree_par = 'admin'`.
...
- **Only an Arena passage is ever held (2026-09-18).** Chapter 11 asks the control to be « léger sur ce qui reste privé et strict sur ce qui devient public », and the free entries it names are the Arena's passages and the face-à-face thesis. A duel take is read by one person, the one who was challenged: it is screened for the record (the verdict stays on `analyses.moderation`, readable if the duel is ever reported) and never held. Holding one broke the duel for both sides, and nobody reviews a duel flag: Roch answered his duel and could not hear the other side, because the filter had held it and the storage policy refuses a take that is not `publiee`. The approval queue of Phase 7 (`en_moderation`) was never in the cahier, which asks for withdrawal after the fact and nothing more, and is gone.
...
- `paire_a_voter()` answers `parle_d_abord` until the caller has spoken, never their own takes, the least shown first, and records the two impressions; each take of a pair carries its `duree_s`. When no pair can be drawn it answers `rien_a_comparer` with `autres`, the number of other published passages (2026-09-18): the screen says « ton passage est le seul », « une seule autre voix, écoute-la dans le classement » or « tu as tout écouté » from that number, instead of « tu as tout écouté » to someone who had heard nothing. `voter()` refuses a vote on one's own take, a pair already voted and a pair from two subjects; it credits `points_par_vote` through the ledger, keyed by the vote id.
...
- `creer_duel(sujet)`, `lire_duel_par_jeton(jeton)` (readable by an anonymous principal: the invitee may not have the app, and never hears the inviter before recording; carries `inviteur_prenom` so the page says who is asking), `rejoindre_duel(jeton)`.
- `mes_duels()` (authenticated, 2026-09-18): the caller's duels as the screens read them, newest first. Each row: the duel, `role` (inviteur or invite), `jeton` for the inviter only (to share the link again), `adversaire {prenom, avatar}` (null until someone joins; the invitee's name is what they gave by the link or their profile's), and two sides `moi` and `lui` from `cote_duel()`: `a_parle`, `prise_id`, `chemin_audio` **exactly where `audio_public_select` would allow it** (their own while it exists, the other's once the duel is closed and that take is `publiee`), `retenue` (the take exists and nobody may play it: withdrawn, held, or its audio already deleted, so the screen says so instead of offering a control that fails), `duree_s`, and `mesures {mots_par_minute, bequilles, silences_tenus}` only once the duel is closed, so nobody reads the other's analysis before having spoken.
- **The second answer closes the duel on the spot** (2026-09-18): `publier_prise()` on a duel take calls `cloturer_duel()` itself when the other take is already there, instead of waiting up to fifteen minutes for `fermer_duels`, which stays as the fallback and the expiry clock.
- **Every event tells the other side**: `rejoindre_duel()` queues `notifier_duel {rejoint}` on the first join, `publier_prise()` queues `{repondu, acteur_id}` when the duel stays open, `cloturer_duel()` queues `{verdict}` or `{expire}`. Keys `duel:<evenement>:<id>[:<acteur>]`, so a retry never tells twice.
- **The two voices stay audible the length of a duel after the verdict**: `cloturer_duel()` sets `date_suppression` to `now() + duree_duel_heures` on a closed duel (an expired one still deletes at once). Before, the audio was marked for deletion at closing and gone within the half hour, before the inviter had opened the app.
...
- Lines `{rang, prise_id, votes, moi, nom, pseudonyme, avatar, duree_s, chemin_audio}`. `nom` is the first name for a person who opted in and for the caller's own line (they know who they are), « Anonyme N » otherwise (2026-09-18: it was « Voix N », and a vote is « une voix », so the ranking read « Voix 2 · 2 voix »; naming it after the recording instead read as a name for something that has none, so the line says what it is, and the number tells two lines apart); `pseudonyme` says which; `avatar` is the picture's path only where the name is a real one. `chemin_audio` (2026-09-18) is the path in `audio-public` only where the caller may hear the passage: their own always, the others' once they have spoken on that subject (`a_parle_sur()`), never once the audio is deleted; `duree_s` is the take's length. The phone draws a crown in gold, silver and bronze on the first three, a play or stop control on every line it may hear, and « (toi) » after the caller's name.
