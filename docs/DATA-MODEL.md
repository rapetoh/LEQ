...
- `paire_a_voter()` answers `parle_d_abord` until the caller has spoken, never their own takes, the least shown first, and records the two impressions; each take of a pair carries its `duree_s`. Every answer carries `ecoutees` and `plafond`, where the listening of the day stands against `prises_ecoutees_par_jour`, so the vote screen draws a real progress instead of numbering pairs against nothing. When no pair can be drawn it answers `rien_a_comparer` with `autres`, the number of other published passages (2026-09-18): the screen says « ton passage est le seul », « une seule autre voix, écoute-la dans le classement » or « tu as tout écouté » from that number, instead of « tu as tout écouté » to someone who had heard nothing. `voter()` refuses a vote on one's own take, a pair already voted and a pair from two subjects; it credits `points_par_vote` through the ledger, keyed by the vote id.
