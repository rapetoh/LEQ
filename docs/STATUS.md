...
is background-only. The command line refused the account again for it. **Build 22 is archived and waiting**: it carries the retimed launch, which Roch found precipitated on build 21, and the microphone prompt rewritten alongside it; the generated `Info.plist` was checked for both the number and the new wording. Its upload is blocked by the Mac's screen lock, the third path closed in one day after the command line ("Failed to Use Accounts" again) and EAS (no free build until 1 October). It waits on Roch unlocking the Mac, or on the App Store Connect API key.
