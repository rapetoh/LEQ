...
Last updated: 2026-09-06, Phase 4 slices 3 and 4 (mobile screens of the path, admin pages of the banks). Update this file in the same commit as any change of state.
...
| 4     | Path machinery, three formats, entitlements, RevenueCat     | Slices 1 to 4 built (database, server, mobile screens, admin banks); simulator tap-through and RevenueCat remain |
...
| `supabase/`        | Five migrations (socle, socle_phase1, demandes_export, parcours, echanger_ordre_defis), idempotent seed with the mockup's static path, pgTAP suites (socle 133, socle_phase1 22, demandes_export 7, parcours 50), CLI config, French auth e-mail templates, a remote test runner and a seed applier                                                                         | pushed to the hosted project on 2026-09-06; every suite green against it |
...
| `apps/admin`       | Vite + React space: login, role gate, layout, home, configuration editor (typed fields, contract validation, optimistic save), flags with confirmation, the banks: défis act by act (create, edit in the three formats, reorder, mark validated, deactivate, rename or add an act) and exercices (create, edit, validate, deactivate)                                       | typecheck, lint, 22 tests                                                |
...
3. Me, without waiting: Phase 5 (streak and points ledgers, D1, D1b, D2, G3 recovery, F1) can start on the same pattern (contract, migration and pgTAP, server, mobile, admin). RevenueCat (`abonnements` writer) waits for the account. The bench corpus layout is ready and the harness runs on the stub; Phase 3 wording and the calibration tool need the Anthropic key and real transcripts.
