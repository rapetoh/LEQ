...
- Supabase project "LEQ" not created: the organisation "rapetoh's Org" is at the free-plan limit of two active projects (`OKiLi`, `voice-expense-tracker`). Roch chooses: pause `OKiLi` if unused, pause `voice-expense-tracker`, or upgrade the organisation to Pro. Free projects also pause after a week of inactivity, which would take LEQ down between sessions; Pro removes that.
- Fly.io: CLI installed, not logged in. `fly auth login` once, then the first deploy and the job-claim verification.
- Device checks of ADR-007: need an iPhone and an Android 14+ device (Google Play account and Android device are an open question).

## Next

1. Roch: choose the Supabase option, run `fly auth login`, answer the Google Play and Android device question, provide the API keys listed in docs/OPEN-INPUTS.md when convenient.
2. Create the project in eu-west-3, `npx supabase link`, `npx supabase db push --include-seed`, register the hook, enable anonymous sign-ins, confirm pg_cron. Fix whatever the first push reveals in the migration and note it here.
