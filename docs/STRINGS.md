...
15. One word, one meaning. A word this application already uses is never given a second meaning on a screen a person reads. "L'analyse" is the measurement of a take and the grid's judgement of it, nothing else; the automatic screening of a published take is "le contrôle automatique". The collision is invisible in a diff and obvious on the screen: "Non contrôlée" on a flagged take was first written "Non analysée", which would have told Rebecca the measures had failed, on a page where that is equally plausible. Before naming a new mechanism, grep the `fr.ts` of that application for the word.
    The second instance, found by Roch on the Arena ranking (2026-09-18): an anonymous passage was named "Voix 2" while a vote is "une voix", so one line read "Voix 2 · 2 voix". The collision is worse than ugly, it makes the number unreadable. A vote is "une voix", and a line with no name says so: "Anonyme N". Naming it after the recording ("Passage N") was the first repair and Roch turned it down, rightly: it reads as a name for something that has none, and "passage" then meant both the recording and its author's label. The same check applies inside one feature: a duel take is "ta réponse" everywhere, so a seat says "Réponse envoyée" and a list line says "à ton tour", never "a parlé" beside "a répondu".
...
| C1, C3, C8     | "voix" for a vote ("31 voix", "62 VOIX SUR 148")                                                                                       | "votes" ("31 votes", "62 votes"). A person with no name is "Anonyme N", so "une voix" is free to mean a human voice again, which is what the vote screen calls its two cards                                                                                                                                                | Rule 15: the mockup used one word for the count and for the person; a vote is the plainer word for what is counted (2026-09-18, Roch)   |
