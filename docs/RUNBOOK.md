...
The worker sends one e-mail on its own: to an invitee who answered a duel by the link and gave their address, at the end of the duel (`notifier_duel`). It goes through the same Gmail account as the sign-in codes: `fly secrets set -a leq-serveur SMTP_HOST=smtp.gmail.com SMTP_PORT=587 SMTP_USER=join.leq@gmail.com SMTP_SENDER=join.leq@gmail.com SMTP_PASS=<the app password of .env>` (set on 2026-09-18). Without them the job logs « aucun serveur SMTP configuré » and the push still leaves. `URL_PUBLIQUE` (default `https://leq-serveur.fly.dev`) is the base of the duel link in that e-mail; set it when the domain exists.
...
`exportOptions.plist` (committed) says `app-store-connect`, destination `upload`, automatic signing. `-allowProvisioningUpdates` let Xcode create the distribution certificate and the App Store profile on the first run. That certificate is cloud-managed: its private key lives behind the signed-in account, and two hours after build 16 the account had gone from the build tools' view (`DVTDeveloperAccountManagerAppleIDLists` empty) and build 17's export failed with "Failed to Use Accounts". An App Store Connect API key (App Store Connect > Users and Access > Integrations > Team Keys, role Admin, the `.p8` downloaded once and kept at the repo root, git-ignored) does not evaporate: pass `-authenticationKeyPath <.p8> -authenticationKeyID <id> -authenticationKeyIssuerID <issuer>` to both commands and no Xcode window is involved at all. Build 19 (2026-09-16) exported and uploaded from the command line again, the day after the account was signed into the released Xcode 27.0, so the failure is the account dropping out of the keychain view, not the tooling. Until the key exists, the way that worked for build 17 and 18 when the command line refused: `open -a /Applications/Xcode.app ios/build/LEQ.xcarchive` opens the archive in the Organizer that sees the account (since Xcode 27.0 was installed on 2026-09-14 that is the released `Xcode.app`; the `Xcode-27-beta6.app` Organizer answers « No Accounts » on the upload sheet, as it did for build 28 on 2026-09-18, while the command-line export still fails with « Failed to Use Accounts »); « Distribute App », « App Store Connect », « Distribute » uploads it with the recommended settings. The clicks are better driven through the accessibility API than with the mouse (2026-09-18): `osascript -e 'tell application "System Events" to tell process "Xcode" to click button "Distribute App" of window "Archives"'`, then the method sheet's `Distribute`, then `click button "Done" of sheet 1 of window "Archives"`. It needs no coordinates, does not depend on which display is attached, works while the window sits behind another app, and can run while Roch is using the Mac. The button lives inside the detail pane, so the path is `button "Distribute App" of scroll area 2 of splitter group 1 of window "Archives"`; the method sheet is remembered from the last upload, so the next click is `button "Distribute" of sheet 1` (or `"Next"` when the sheet reopens on its progress). The same interface reads the progress without a screenshot: `get name of every UI element of sheet 1 of window "Archives"` walks from « Creating LEQ.ipa… » to « Upload completed with warnings » or « Upload failed with errors ». **The archive itself records the outcome**, which is the check that does not depend on a window being visible: `/usr/libexec/PlistBuddy -c Print ~/Library/Developer/Xcode/Archives/<date>/<nom>.xcarchive/Info.plist` holds a `Distributions` array whose `uploadEvent` carries `shortTitle = Uploaded` and an `errors` array, next to the `CFBundleVersion` it was. Apple's side fails transiently (build 31 died on « xcodebuild encountered an error (69) » after « Checking completion status », then went through unchanged on the next attempt), so read that record before concluding anything and retry once before looking further. `cliclick` from a `screencapture -x` still works as a fallback and is what earlier builds used. One trap: a result sheet left open blocks the next distribution and stops answering both clicks and keystrokes after a while (one sat for nine hours and swallowed everything until Xcode was restarted), so dismiss it in the same session as the upload. Two details learned on builds 20 and 21: click the archive row first, because the Organizer window is not key when Xcode opens it and the first click on « Distribute App » is swallowed; and send the clicks as a move, a pause, a press and a release (`cliclick m:x,y w:300 dd:x,y w:120 du:x,y`), because a plain `c:` click did not always register on those buttons. With the Organizer at its default place on this display, « Distribute App » sits at (1338, 262), « Distribute » in the sheet at (1271, 747) and « Done » at (1273, 747); the upload takes about two minutes and ends on « Upload completed with warnings », the dSYM warnings for the prebuilt React, Hermes and ffmpeg frameworks. Three more from build 26: those positions hold only on the 1920 × 1080 virtual display that Chrome Remote Desktop adds when Roch drives the Mac from another device (`system_profiler SPDisplaysDataType` lists it next to the built-in Retina panel), and the point width can change between two commands, so move the pointer and `screencapture -C -x` to see where it lands before clicking; a click that falls on the wallpaper hides every window (macOS « click wallpaper to reveal desktop »), and the window comes back with the next `open`; and each `open` of the `.xcarchive` registers one more copy of it in the Organizer, so open it once per build. The archive takes about ten minutes, the upload one; the build number comes from `ios.buildNumber` in `app.json` and must exceed the last one on App Store Connect. Before any build that touches audio, open `leq://diagnostic` on the simulator (development client on Metro 8082) and read its lines: it runs the recording chain and the face-à-face's audio start; build 27 exists because that path had never been executed on iOS. When another session has uncommitted files in the working tree, archive from a worktree at the commit to ship: `git worktree add /Users/roch/Desktop/LEQ-build<N> HEAD`, symlink the root `node_modules` (and each workspace's, where present) into it, copy each `dist/` of `packages/*` and the two `.env` files, then prebuild and archive there; remove the worktree afterwards with `git worktree remove`. The export prints "Upload Symbols Failed" warnings for the prebuilt React, Hermes and ffmpeg frameworks, which ship without dSYMs; that only affects crash symbolication inside those frameworks. Metro inlines `apps/mobile/.env` at archive time, so a local build carries the Google client ids and the PostHog key without any EAS environment variable. With an App Store Connect API key instead of a signed-in account, add `-authenticationKeyPath`, `-authenticationKeyID` and `-authenticationKeyIssuerID` to both commands.

**« Failed to Use Accounts » and the locked screen (2026-09-19).** Build 35 exported and uploaded
from the command line at 13:16. Ten minutes later, with nothing changed in the repo or in Xcode,
the same command refused build 36 twice with « Failed to Use Accounts », and the Mac was locked
with its screensaver running. The signing identity is cloud-managed and its private key lives in
the login keychain, which locks with the session, so the account is invisible to the build tools
exactly while the screen is locked. That fits every earlier occurrence, all of them in the evening
or with Roch away. So the first thing to check when the export refuses the account is not Xcode
but the lock:

```bash
python3 -c "import subprocess, plistlib; d=plistlib.loads(subprocess.run(['ioreg','-n','Root','-d1','-a'],capture_output=True).stdout); print(d[0].get('IOConsoleLocked'))"
```

`true` means the upload cannot go out by any path: the Organizer is no help either, because System
Events reports zero windows for Xcode while the session is locked (and `tell application "Xcode"`
answers « AppleEvent timed out »), which looks exactly like a wedged Xcode and is not one. Waiting
...
A string is never reviewed alone. Read by itself in `fr.ts` a sentence looks fine; the faults are
in what sits beside it. Every one Roch has caught was of that kind, and every one was readable in
the code:

- a card whose footer repeated its own body ("La formule Gratuit donne un défi par jour" above
  "Formule Gratuit · un défi par jour");
- an account step whose title was also its field label ("Ton adresse e-mail", twice);
- one shared line that agreed with one title and not the other ("Se réservent chez Rebecca" under
  "Une heure en tête-à-tête").

So the review is a code read, and it needs no simulator:

```bash
npm run ecrans              # every screen, with the strings it renders, in render order
...
`npm run strings` refuses known bad shapes, and this shows composition, which no regular
expression can judge.

A screen that renders nothing but groups or formatted values is not listed, which is why the
count is lower than the number of files.

### Seeing it rendered, when layout is in question

Only layout faults need pixels: truncation, overflow, a line clamped mid-word. `simctl` does it
without the GUI session, so it works while the Mac is locked:

```bash
xcrun simctl boot <UDID>                                   # a locked screen is fine
...
Two limits. There is no tap: `idb` is not installed and a `leq://` link raises an iOS
confirmation the command line cannot answer, so only the screen the app opens on can be read; to
reach another, Metro has to be started with `EXPO_PUBLIC_ECRAN_INITIAL="/(onglets)/moi"` (read in
`src/app/index.tsx`, development only, inlined at bundle time, so it needs its own bundler on a
free port). And an empty account hides most faults, because the strings that go wrong are the ones
that only render when there is data.
