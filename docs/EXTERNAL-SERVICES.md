...
| ---------------------------------------------------------------------------- | ------------------------------------------------------------- | -------------------------- | -------------------------------------------------- | ------------------------------------------------- | -------------------------------------- |
| Supabase                                                                     | Postgres, Auth, Storage, Realtime, pg_cron, Edge Functions    | Roch (org "rapetoh's Org") | Org exists, project "LEQ" not created (blocked)    | eu-west-3, Paris                                  | Needed, available in the dashboard     |
| Fly.io                                                                       | The Node service (worker, temps-reel)                         | Roch                       | CLI installed, not logged in, app not created      | cdg, Paris                                        | Needed, published by Fly (verify link) |
...
- Account: organization "rapetoh's Org" owned by Roch, two existing projects (`OKiLi`, `voice-expense-tracker`).
- Status: project "LEQ" not created. The owner is at the limit of two active free projects (docs read on 2026-09-06: two active free projects per owner across organizations, paused projects do not count). Options in docs/STATUS.md.
- Region: eu-west-3 (Paris, AWS). Chosen at project creation, cannot change afterwards.
